...
if ((!empty($room)) or (isset($area_id))) {
    ...
    if ((authGetUserLevel(getUserName(),$area_id,"area") >=4) or (authGetUserLevel(getUserName(),$room) >=4)) {
        echo "<input type=\"text\" name=\"room_name\" size=\"40\" value=\"".my_htmlspecialcharacters($row["room_name"])."\" />\n";
    } else {
        echo "<input type=\"hidden\" name=\"room_name\" value=\"".my_htmlspecialcharacters($row["room_name"])."\" />\n";
        echo "<b>".my_htmlspecialcharacters($row["room_name"])."</b>\n";
    }
    echo "</td></tr>\n";
    // Description
    echo "<tr><td>".get_vocab("description")."</td><td><input type=\"text\" name=\"description\"  size=\"40\" value=\"".my_htmlspecialcharacters($row["description"])."\" /></td></tr>\n";
    // Domaine
      ...
      if ($nb_area > 1) {
        ...
        <option value=\"-1\">".get_vocab('choose_an_area')."</option>\n";
          for ($enr = 0; ($row1 = grr_sql_row($res, $enr)); $enr++) {
            ...
            echo '>'.my_htmlspecialcharacters($row1[1]);
            echo '</option>'."\n";
          }
          grr_sql_free($res);
          echo "</select></td></tr>";
    ...
    echo "<td><input type=\"text\" name=\"area_order\" size=\"1\" value=\"".my_htmlspecialcharacters($row["order_display"])."\" /></td>\n";
    echo "</tr>\n";
...
<?php
    ...
    if ((authGetUserLevel(getUserName(),$area_id,"area") >=4) or (authGetUserLevel(getUserName(),$room) >=4)) {
        ...
        <b>".my_htmlspecialcharacters($row["max_booking"])."</b>
        </td></tr>";
    }
...
if ((!empty($id_area)) or (isset($add_area)))
{
  ...
  if (isset($id_area)) {
    ...
    echo "<td style=\"width:30%;\"><input type=\"text\" name=\"area_name\" size=\"40\" value=\"".my_htmlspecialcharacters($row["area_name"])."\" /></td>\n";
    echo "</tr><tr>\n";
    // Ordre d'affichage du domaine
    ...
    if (getSettingValue("module_multisite") == "Oui") {
      ...
      if ($nb_site > 1) {
        ...
        for ($enr = 0; ($row1 = grr_sql_row($res, $enr)); $enr++) {
          ...
          if ($id_site == $row1[0])
            echo ' selected="selected"';
          echo '>'.my_htmlspecialcharacters($row1[2]);
          echo '</option>'."\n";
        }
        grr_sql_free($res);
        echo "</select></td></tr>";
    ...
    if (OPTION_IP_ADR==1) {
        ...
        echo "<td><input type=\"text\" name=\"ip_adr\" value=\"".my_htmlspecialcharacters($row["ip_adr"])."\" /></td>\n";
        echo "</tr>\n";
        echo "<tr>\n";
    ...
    echo "<td><input type=\"text\" name=\"eveningends_minutes_area\" size=\"5\" value=\"".my_htmlspecialcharacters($row["eveningends_minutes_area"])."\" /></td>\n";
    echo "</tr>";

    // Resolution - quel bloc peut être réservé, en secondes
    echo "<tr>\n";
    echo "<td>".get_vocab("resolution_area").get_vocab("deux_points")."</td>\n";
    echo "<td><input type=\"text\" name=\"resolution_area\" size=\"5\" value=\"".my_htmlspecialcharacters($row["resolution_area"])."\" /></td>\n";
    echo "</tr>";
    ...
    echo "<td><input type=\"text\" name=\"duree_par_defaut_reservation_area\" size=\"5\" value=\"".my_htmlspecialcharacters($row["duree_par_defaut_reservation_area"])."\" /></td>\n";
    echo "</tr>";
