    ...
    if ($room !=-1) {
        ...
        if ($test != "0") {
            ...
            if ($valeur != '') {
                ...
                if (grr_sql_command($sql) < 0) {fatal_error(0, "<p>" . grr_sql_error());}  else {$msg=get_vocab("add_multi_user_succeed");}
            }
        }
    } else {
        ...
        if ($res) for ($i = 0; ($row = grr_sql_row($res, $i)); $i++)
        {
            ...
            if ($nb==0) {
                ...
                if (grr_sql_command($sql3) < 0) {fatal_error(0, "<p>" . grr_sql_error());}  else {$msg=get_vocab("add_multi_user_succeed");}
            }
        }
    }
  }	
